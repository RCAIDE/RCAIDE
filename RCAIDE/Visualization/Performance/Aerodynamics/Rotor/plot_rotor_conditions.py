...
from RCAIDE.Visualization.Performance.Common import set_axes, plot_style, compile_propulsor_groups
...
def plot_rotor_conditions(results,
    ...
    pg = compile_propulsor_groups(results)
                ...
                for bus in busses: 
                    ...
                    for j in range(len(active_propulsor_groups)):  
                        if pg[pg_i] == active_propulsor_groups[j]:
                            ...
                            axes_1.plot(time,rpm, color = line_colors[i], marker = ps.markers[j], line_style =  ps.line_style , linewidth = ps.line_width, label = segment_name)
                            axes_1.set_ylabel(r'RPM')
                            set_axes(axes_1)    
                            
                            axes_2 = plt.subplot(3,2,2)
                            axes_2.plot(time, tm, color = line_colors[i], marker = ps.markers[j], line_style =  ps.line_style , linewidth = ps.line_width) 
                            axes_2.set_ylabel(r'Tip Mach')
                            set_axes(axes_2) 
                    
                            axes_3 = plt.subplot(3,2,3)
                            axes_3.plot(time,thrust, color = line_colors[i], marker = ps.markers[j], line_style =  ps.line_style , linewidth = ps.line_width)
                            axes_3.set_ylabel(r'Thrust (N)')
                            set_axes(axes_3) 
                            
                            axes_4 = plt.subplot(3,2,4)
                            axes_4.plot(time,torque, color = line_colors[i], marker = ps.markers[j], line_style =  ps.line_style , linewidth = ps.line_width)
                            axes_4.set_ylabel(r'Torque (N-m)')
                            set_axes(axes_4)    
                            
                            axes_5 = plt.subplot(3,2,5)
                            axes_5.plot(time, Cp, color = line_colors[i], marker = ps.markers[j], line_style =  ps.line_style , linewidth = ps.line_width)
                            axes_5.set_xlabel('Time (mins)')
                            axes_5.set_ylabel(r'Power Coefficient')
                            set_axes(axes_5) 
                    
                            axes_6 = plt.subplot(3,2,6)
                            axes_6.plot(time, eta, color = line_colors[i], marker = ps.markers[j], line_style =  ps.line_style , linewidth = ps.line_width)

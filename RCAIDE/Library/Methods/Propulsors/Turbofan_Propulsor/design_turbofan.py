...
def design_turbofan(turbofan):
    ...
    sls_conditions.energy.distribution_lines[fuel_line.tag]                                  = RCAIDE.Framework.Mission.Common.Conditions()
    sls_conditions.energy.distribution_lines[fuel_line.tag].fuel_tanks                       = RCAIDE.Framework.Mission.Common.Conditions()
    sls_conditions.energy.distribution_lines[fuel_line.tag].fuel_tanks[fuel_tank.tag]                   = RCAIDE.Framework.Mission.Common.Conditions()  
    sls_conditions.energy.distribution_lines[fuel_line.tag].fuel_tanks[fuel_tank.tag].mass_flow_rate    = np.zeros((1,1))     
    sls_conditions.energy.distribution_lines[fuel_line.tag].fuel_tanks[fuel_tank.tag].mass              = np.zeros((1,1))
    sls_conditions.energy.distribution_lines[fuel_line.tag].propulsors                = RCAIDE.Framework.Mission.Common.Conditions() 
    sls_conditions.energy.distribution_lines[fuel_line.tag].propulsors[turbofan.tag]                    = RCAIDE.Framework.Mission.Common.Conditions() 
    sls_conditions.energy.distribution_lines[fuel_line.tag].propulsors[turbofan.tag].throttle           = np.array([[1.0]])
    sls_conditions.energy.distribution_lines[fuel_line.tag].propulsors[turbofan.tag].y_axis_rotation    = np.zeros((1,1)) 
    sls_conditions.energy.distribution_lines[fuel_line.tag].propulsors[turbofan.tag].thrust             = np.zeros((1,1))
    sls_conditions.energy.distribution_lines[fuel_line.tag].propulsors[turbofan.tag].power              = np.zeros((1,1))
    

    # initialize data structure for turbofan operating conditions (for noise )       
    sls_conditions.noise.distribution_lines[fuel_line.tag]                             = RCAIDE.Framework.Mission.Common.Conditions()
    sls_conditions.noise.distribution_lines[fuel_line.tag].propulsors                       = RCAIDE.Framework.Mission.Common.Conditions()
    sls_conditions.noise.distribution_lines[fuel_line.tag].propulsors[turbofan.tag]                   = RCAIDE.Framework.Mission.Common.Conditions() 
    sls_conditions.noise.distribution_lines[fuel_line.tag].propulsors[turbofan.tag].turbofan          = RCAIDE.Framework.Mission.Common.Conditions()

    ...
    if len(rcaide_wing.Segments.keys())>0: 
        for i_segs in range(n_segments):
            if (i_segs == n_segments-1):
                ...
                if segments[segment_names[i_segs]].sweeps.leading_edge is not None: 
                    # If leading edge sweep is defined 
                    sweep       = segments[segment_names[i_segs]].sweeps.leading_edge  
                else:   
                    # If quarter chord sweep is defined, convert it to leading edge sweep
                    sweep_quarter_chord = segments[segment_names[i_segs]].sweeps.quarter_chord 
                    chord_fraction      = 0.25                          
                    segment_root_chord  = root_chord*segments[segment_names[i_segs]].root_chord_percent
                    segment_tip_chord   = root_chord*segments[segment_names[i_segs+1]].root_chord_percent
                    segment_span        = semispan*(segments[segment_names[i_segs+1]].percent_span_location - segments[segment_names[i_segs]].percent_span_location )
            ...
            dihedral       = segments[segment_names[i_segs]].dihedral_outboard   
    
            # append section 
            section        = Section() 
            section.tag    = segments[segment_names[i_segs]].tag
            section.chord  = root_chord*segments[segment_names[i_segs]].root_chord_percent 
            section.twist  = segments[segment_names[i_segs]].twist/Units.degrees    
            section.origin = origin # first origin in wing root, overwritten by section origin 
            if segments[segment_names[i_segs]].Airfoil:
                if segments[segment_names[i_segs]].Airfoil.airfoil.coordinate_file is not None:
                    section.airfoil_coord_file   = write_avl_airfoil_file(segments[segment_names[i_segs]].Airfoil.airfoil.coordinate_file)
                elif segments[segment_names[i_segs]].Airfoil.airfoil.naca_airfoil is not None:
                    section.naca_airfoil         = segments[segment_names[i_segs]].Airfoil.airfoil.naca_airfoil     
            ...
            if (i_segs == n_segments-1):
                ...
                if getattr(rcaide_wing,'control_surfaces',False):   
                    root_chord_percent = segments[segment_names[i_segs]].root_chord_percent
                    tip_chord_percent  = segments[segment_names[i_segs+1]].root_chord_percent
                    tip_percent_span   = segments[segment_names[i_segs+1]].percent_span_location
                    root_percent_span  = segments[segment_names[i_segs]].percent_span_location
                    root_twist         = segments[segment_names[i_segs]].twist
                    tip_twist          = segments[segment_names[i_segs+1]].twist
                    tip_airfoil        = segments[segment_names[i_segs+1]].Airfoil 
                    seg_tag            = segments[segment_names[i_segs+1]].tag 
            ...
            segment_percent_span =    segments[segment_names[i_segs+1]].percent_span_location - segments[segment_names[i_segs]].percent_span_location     

...
# ----------------------------------------------------------------------------------------------------------------------  
def supersonic_compressibility_drag_total(state,settings,geometry):
    ...
    Cl_wings         = conditions.aerodynamics.coefficients.lift.breakdown.compressible_wings   
    ...
    # Use vehicle wings  reference area for drag coefficients
    ...
    for wing in geometry.wings:
        low_cutoff_volume_total += drag_div(low_mach_cutoff*np.ones([1]), wing, Cl_wings[wing.tag], Sref_main)[0]
    ...
    for wing in geometry.wings:
        cd_c_v_base[low_inds] += drag_div(Mach[low_inds], wing, Cl_wings[wing.tag][low_inds], Sref_main)[0]
    ...
    # Save drag breakdown 
    drag = conditions.aerodynamics.coefficients.drag.breakdown 
    drag.compressible = Data(
        total        = cd_c, 
        total_volume = cd_c_v, 
        total_lift   = cd_c_l,
        ) 

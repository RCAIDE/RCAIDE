...
def parasite_total(state,settings,geometry):
    ...
    for wing in geometry.wings:
        parasite_drag = conditions.aerodynamics.coefficients.drag.breakdown.parasite[wing.tag].total 
        conditions.aerodynamics.coefficients.drag.breakdown.parasite[wing.tag].total  = parasite_drag * wing.areas.reference/vehicle_reference_area
    ...
    for fuselage in geometry.fuselages:
        if type(fuselage) == RCAIDE.Library.Components.Fuselages.Blended_Wing_Body_Fuselage:
            continue
        parasite_drag = conditions.aerodynamics.coefficients.drag.breakdown.parasite[fuselage.tag].total 
        conditions.aerodynamics.coefficients.drag.breakdown.parasite[fuselage.tag].total = parasite_drag * fuselage.areas.front_projected/vehicle_reference_area
            ...
            for bus in network.busses:
                for propulsor in bus.propulsors:  
                    if 'nacelle' in propulsor:
                        ...
                        parasite_drag = conditions.aerodynamics.coefficients.drag.breakdown.parasite[nacelle.tag].total 
                        conditions.aerodynamics.coefficients.drag.breakdown.parasite[nacelle.tag].total  = parasite_drag * ref_area/vehicle_reference_area
                        total_parasite_drag += parasite_drag * ref_area/vehicle_reference_area 
                            
                        if nacelle.has_pylon:
                            parasite_drag = conditions.aerodynamics.coefficients.drag.breakdown.parasite[nacelle.tag + '_pylon'].total 
                            conditions.aerodynamics.coefficients.drag.breakdown.parasite[nacelle.tag + '_pylon'].total = parasite_drag * fuselage.areas.front_projected/vehicle_reference_area
                            total_parasite_drag += parasite_drag * ref_area/vehicle_reference_area 

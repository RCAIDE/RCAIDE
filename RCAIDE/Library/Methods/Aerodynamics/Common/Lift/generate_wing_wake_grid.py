...
def generate_wing_wake_grid(vehicle, H, L, H_f, x_plane, N_z_coarse=20, N_z_fine=35, N_y_coarse=20):
    ...
    z_outer_bot   = rp.linspace(z_bot, -H_f, Nzo_half)
    z_outer_top   = rp.linspace(H_f, z_top, Nzo_half)
    
    # Use finer concentration of grid points near the wing
    z_inner_bot   = -H_f*(rp.flipud((1-rp.cos(rp.linspace(1e-6,1,N_z_fine)*rp.pi/2))))
    z_inner_top   = H_f*(1-rp.cos(rp.linspace(0,1,N_z_fine)*rp.pi/2))
    zlocs         = rp.concatenate([z_outer_bot,z_inner_bot, z_inner_top, z_outer_top])
    ...
    if L < 1.:
        ...
        y_outerspan = rp.linspace(1.01,L,Nyo_half)*half_span
        y_semispan  = rp.append(y_semispan, y_outerspan)
        
    ylocs = rp.concatenate([rp.flipud(-y_semispan),y_semispan])
    
    # declare new control points
    cp_YC = rp.repeat(ylocs,len(zlocs)) 
    cp_ZC = rp.tile(zlocs,len(ylocs))
    cp_XC = rp.ones_like(cp_YC)*x_plane 

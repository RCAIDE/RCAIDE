...
def VLM(conditions,settings,geometry, viscous_VLM_flag=False):
    ...
    if viscous_VLM_flag: 
        ...
        while np.any(delta_alpha_i>tol):
            
            # while convergence is not met    
         
            # update alpha_i (eqn 8)
         
            # update beta_i (eqn 9)
         
            # Solve the VLM
            RESULTS =  VLM_Routine(VD,conditions,settings,geometry, x_m, z_m, S_ref, b_ref,c_bar)
            
            
            # compute alpha_ind for each section
            
            # compute CL effi and Cf eff i
            
            # compute Cl visc
    
            # # create arrays of spanwise CLs and CDs
            
            
            # # compute CLs_2D and CDs_2D at wing sections
            
            
            # # interpolate to find CLs_2D and CDs_2D along entire wings
            ...
            # # compute kappa (eqn 21)
            
            # # Compute effective sweep (eqn 18)
            
            # # Compute F (eqn 20)
            
            # # compute CL_2.5D and AoA2.5D (eqn 19)
                       
            
            # update delta alpha i 
...
def VLM_Routine(VD,conditions,settings,geometry, x_m, z_m, S_ref, b_ref,c_bar):
    ...
    VD.XBAR = np.ones(sum(LE_ind)) * x_m
    VD.ZBAR = np.ones(sum(LE_ind)) * z_m
    ...
    SPC  = VD.K_SPC*np.ones_like(DCP_LE)
    ...
    results   = compute_induced_drag(Clift_y, aoa, X, Y, Z, CHORD_strip, SURF,VD.n_sw,S_ref)    
    ...
    results.CM                = CM  
    results.CN                = CN 
    results.chord_sections    = VD.n_cp

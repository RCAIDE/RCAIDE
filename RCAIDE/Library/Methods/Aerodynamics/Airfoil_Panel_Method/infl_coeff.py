...
def infl_coeff(x,y,xbar,ybar,st,ct,npanel,ncases,ncpts):
    ...
    ainfl                = rp.zeros((ncases,ncpts,npanel+1,npanel+1))    
    pi2inv               = 1 / (2*rp.pi) 
    r_ij                 = rp.zeros((ncases,ncpts,npanel,npanel+1)) #(AOAs, REs, npanels, no. of nodes)
    ...
    diag_indices         = list(rp.tile(rp.repeat(rp.arange(npanel),ncases),ncpts))
    aoas                 = list(rp.tile(rp.arange(ncases),ncpts*npanel))
    res                  = list(rp.repeat(rp.arange(ncpts),ncases*npanel))   

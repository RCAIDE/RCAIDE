...
def fuselage_planform(fuselage, circular_cross_section = True):
    ...
    if len(fuselage.segments) > 2:
        ...
        for i in range(len(fuselage.segments)-1):
            ...
            delta_x                = fuselage.length * (seg_2.percent_x_location -  seg_1.percent_x_location)
            side_projected_area   += ((seg_1.height +  seg_2.height ) / 2) * delta_x 
            wetted_area           += MADISON ADD CODE HERE 
            
            A_1  = np.pi *  (seg_1.height / 2) *  (seg_1.weight / 2)   
            A_2  = np.pi *  (seg_2.height / 2) *  (seg_2.weight / 2)    

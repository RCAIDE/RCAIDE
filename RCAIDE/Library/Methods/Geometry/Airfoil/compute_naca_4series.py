...
def compute_naca_4series(airfoil_code,npoints= 201, leading_and_trailing_edge_resolution_factor = 1.5):
    ...
    x_us  = rp.linspace(0,1,int(rp.ceil(half_npoints)))   # number of points per side    
    ...
    if camber == 0 and camber_loc == 0:
        ...
        I_us = rp.where(x_us<camber_loc)[0] 
        I_ls = rp.where(x_ls<camber_loc)[0]  
    ...
    airfoil_data.y_upper_surface    = rp.append(0,y_up_surf) 

...
def func_propeller_design(prop,number_of_stations=20):
...
def func_objective(x,airfoils,a_loc,RE,Cl,N):
    # query surrogate for sectional Cls at stations 
    Cl_vals          = np.zeros(N)       
    for j,airfoil in enumerate(airfoils): 
        pd            = airfoil.polars
...
def propeller_design(State, Settings, System):
	...
	results = func_propeller_design('prop', 'number_of_stations')
...
def objective(State, Settings, System):
	...
	results = func_objective('x', 'airfoils', 'a_loc', 'RE', 'Cl', 'N')
	#TODO: [Replace results with the output of the original function]

...
def evaluate_surrogate(state,settings,geometry):
    ...
    Args:
        stability  : Vortex Lattice Method analysis          [unitless]
        state      : flight conditions     [unitless]
        settings   : Vortex Lattice Method analysis settings [unitless]
    ...
    conditions.aerodynamics.coefficients.lift.total                      = conditions.static_stability.coefficients.lift # overwrite lift in aerodynamic results 
    conditions.aerodynamics.coefficients.lift.breakdown.total            = conditions.static_stability.coefficients.lift # overwrite lift in aerodynamic results 
    conditions.aerodynamics.coefficients.drag.breakdown.induced.inviscid = conditions.static_stability.coefficients.drag 
...
def evaluate_no_surrogate(state,settings,geometry):
    """Evaluates forces and moments directly using Vortex Lattice Method.
...
def sample_training(stability):
    """Call methods to run Vortex Lattice Method for sample point evaluation. 
...
# ----------------------------------------------------------------------
#  Evaluate Vortex Lattice Method
# ----------------------------------------------------------------------
def evaluate_VLM(conditions,settings,geometry):
    """Calculate stability coefficients inluding specific wing coefficients using the Vortex Lattice Method
    ...
    Returns: 
        None  
    """    
    results = Vortex_Lattice_Method(conditions,settings,geometry)
    Clift   = results.CL       
    Cdrag   = results.CDi      
    CX      = results.CX       
    CY      = results.CY       

...
class Turbofan_Engine_Network(Network):
    ...
    def add_unknowns_and_residuals_to_segment(self, segment):
        ...
        for fuel_line_i, fuel_line in enumerate(fuel_lines):    
            ...
            # ------------------------------------------------------------------------------------------------------
            segment.state.conditions.energy[fuel_line.tag]       = RCAIDE.Frameworks.Mission.Common.Conditions()       
            fuel_line_results                                    = segment.state.conditions.energy[fuel_line.tag]    
            segment.state.conditions.noise[fuel_line.tag]        = RCAIDE.Frameworks.Mission.Common.Conditions()  
            noise_results                                        = segment.state.conditions.noise[fuel_line.tag]      
     
            for fuel_tank in fuel_line.fuel_tanks:               
                fuel_line_results[fuel_tank.tag]                 = RCAIDE.Frameworks.Mission.Common.Conditions()  
            ...
            # ------------------------------------------------------------------------------------------------------
            for turbofan in fuel_line.propulsors:               
                fuel_line_results[turbofan.tag]                         = RCAIDE.Frameworks.Mission.Common.Conditions() 
                ...
                noise_results[turbofan.tag]                             = RCAIDE.Frameworks.Mission.Common.Conditions() 
                noise_results[turbofan.tag].turbofan                    = RCAIDE.Frameworks.Mission.Common.Conditions() 

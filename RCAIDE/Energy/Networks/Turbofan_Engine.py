...
class Turbofan_Engine(Network):
    ...
    def unpack_unknowns(self,segment):
        ...
        fuel_lines   = segment.analyses.energy.network.turbofan_engine.fuel_lines  # segment.analyses.energy.networks.turbofan_engine.fuel_lines  
        for fuel_line_i, fuel_line in enumerate(fuel_lines):            
            if (type(segment) == RCAIDE.Analyses.Mission.Segments.Ground.Takeoff):
                ...
                fuel_line_results.throttle  = segment.state.unknowns.throttle # segment.state.unknowns[fuel_line.tag + '_throttle']  
    ...
    def add_unknowns_and_residuals_to_segment(self, segment):
        ...
        for fuel_line_i, fuel_line in enumerate(fuel_lines):    
            ...
            for propulsor in fuel_line.propulsors:               
                fuel_line_results[propulsor.tag]                         = Conditions()
                fuel_line_results[propulsor.tag].turbofan                = Conditions()  
                ...
                noise_results[propulsor.tag]                             = Conditions() 
                noise_results[propulsor.tag].turbofan                    = Conditions() 

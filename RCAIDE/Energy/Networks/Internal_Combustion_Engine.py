...
class Internal_Combustion_Engine(Network):
    ...
    def unpack_unknowns(self,segment):
        ...
        for fuel_line in fuel_lines: 
            ...
            for i in range(len(active_propulsor_groups)): 
                ...
                fuel_line_results[pg_tag].throttle  = segment.state.unknowns[fuel_line.tag + '_' + pg_tag + '_throttle']  
    ...
    def add_unknowns_and_residuals_to_segment(self,
        ...
        for fuel_line_i, fuel_line in enumerate(fuel_lines):  
            ...
            # ------------------------------------------------------------------------------------------------------
            segment.state.conditions.energy[fuel_line.tag] = RCAIDE.Analyses.Mission.Segments.Conditions.Conditions()            
            ...
            for fuel_tank in fuel_tanks:               
                fuel_line_results[fuel_tank.tag]                 = RCAIDE.Analyses.Mission.Segments.Conditions.Conditions()  
            ...
            for i in range(len(sorted_propulsors.unique_rotor_tags)):        
                ...
                pg_tag                                            = active_propulsor_groups[i] 
                fuel_line_results[pg_tag]                         = RCAIDE.Analyses.Mission.Segments.Conditions.Conditions()
                fuel_line_results[pg_tag].engine                  = RCAIDE.Analyses.Mission.Segments.Conditions.Conditions()
                fuel_line_results[pg_tag].rotor                   = RCAIDE.Analyses.Mission.Segments.Conditions.Conditions() 

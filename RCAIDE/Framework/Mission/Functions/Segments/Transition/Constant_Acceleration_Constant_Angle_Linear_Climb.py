...
def initialize_conditions(segment):
    ...
    elif len(rp.shape(v0)) == 0:
        v0 = rp.array([v0, 0, 0]) 
         
    # discretize on altitude
    v0_mag          = rp.linalg.norm(v0)
    alt             = t_nondim * (altf-alt0) + alt0   
    ground_distance = (altf-alt0)/rp.tan(climb_angle)
    true_distance   = rp.sqrt((altf-alt0)**2 + ground_distance**2)
    t_initial       = conditions.frames.inertial.time[0,0]    
    elapsed_time    = (-v0_mag + rp.sqrt(v0_mag**2 + 2*ax*true_distance))/(ax)   
    ...
    # Figure out vx
    V  = (vf_mag-v0_mag) 
    vx = t_nondim *  V  * rp.cos(beta) * rp.cos(climb_angle) + v0[0] * rp.cos(climb_angle) 
    vy = t_nondim *  V  * rp.sin(beta) * rp.cos(climb_angle) + v0[1] * rp.cos(climb_angle) 
    vz = t_nondim *  V  * rp.sin(climb_angle) + v0[2] * rp.sin(climb_angle)  

...
# ----------------------------------------------------------------------------------------------------------------------      
## @ingroup Methods-Noise-Certification 
def turbofan_flyover_noise(takeoff_mission,noise_configs):  
    ...
    if mission.npoints_takeoff_sign == -1:
        noise_result_takeoff_FL_clb = 500. + noise_segment.missions.sideline_takeoff.segments.climb.state.numerics.number_of_control_points
    else:    
        noise_result_takeoff_FL_clb = compute_turbofan_aircraft_noise(noise_config,noise_analyses,noise_segment,noise_settings)   
    ...
    if mission.npoints_takeoff_sign == -1:
        noise_result_takeoff_FL_cutback = 500. + noise_segment.missions.sideline_takeoff.segments.climb.state.numerics.number_of_control_points
    else:                                       
        noise_result_takeoff_FL_cutback = compute_turbofan_aircraft_noise(noise_config,noise_analyses,noise_segment,noise_settings)   

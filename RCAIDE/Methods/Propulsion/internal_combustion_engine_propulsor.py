...
def compute_propulsor_performance(i,network_tag,propulsor_group_tag,engines,propellers,N_propellers,state,): 
    ...
    N.A.        
    '''
    ice_net__results    = state.conditions.energy[network_tag]
    ...
    # Throttle the engine
    engine.inputs.speed                                     = state.conditions.energy.propulsor_group_0.propeller.rpm * Units.rpm
    state.conditions.energy.comice_net_tion_engine_throttle = state.conditions.energy.throttle
    
    # Run the engine
    engine.power(state.conditions)
    mdot         = mdot + engine.outputs.fuel_flow_rate * N_propellers[i]
    torque       = engine.outputs.torque     
    
    # link
    propeller.inputs.omega = state.conditions.energy.propulsor_group_0.propeller.rpm * Units.rpm 

    # Spin the propeller 
    F, Q, P, Cp, outputs, etap = propeller.spin(state.conditions) 

    # Check to see if magic thrust is needed, the ESC caps throttle at 1.1 already
    eta                 = ice_net__results[propulsor_group_tag].throttle  
    P[eta>1.0]          = P[eta>1.0]*eta[eta>1.0]
    F[eta[:,0]>1.0,:]   = F[eta[:,0]>1.0,:]*eta[eta[:,0]>1.0,:]

    # Determine Conditions specific to this instantation of engine and propellers

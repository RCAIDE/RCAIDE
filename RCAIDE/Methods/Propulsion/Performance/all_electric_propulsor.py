...
def compute_propulsor_performance(bus,state,voltage):
    ...
    for propulsor_index, propulsor in enumerate(bus.propulsors): 
        if bus.identical_propulsors == True and propulsor_index>0: 
            ...
            energy_results.esc.throttle            = energy_results_0.esc.throttle          
            ...
            esc.calculate_voltage_out_from_throttle(energy_results.motor.throttle)
            esc.inputs.throttle  = energy_results.motor.throttle
            ...
            eta                = energy_results.motor.throttle 
            F[eta[:,0]  <=0.0] = 0.0
            P[eta[:,0]  <=0.0] = 0.0
            Q[eta[:,0]  <=0.0] = 0.0 

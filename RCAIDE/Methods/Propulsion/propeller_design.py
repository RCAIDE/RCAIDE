...
def propeller_design(prop, tol=1e-10):
    ...
    # ---------------------------------------------------------------------------------------------------------
    # Checks for over-specification, nondimensionalized performance metrics
    # ---------------------------------------------------------------------------------------------------------
    ...
    # Check for user defined airfoils, and dimension of stations
    if len(a_loc) != N:
...
def objective(x,airfoil_analyses,a_loc,RE,Cl,N):
    # query surrogate for sectional Cls at stations 
    Cl_vals          = np.zeros(N)       
    for j, airfoil_polar_data in enumerate(airfoil_analyses.airfoil_data): 
        pd            = airfoil_polar_data

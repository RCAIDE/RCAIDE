...
from RCAIDE.Methods.Weights.Buildups.Common   import compute_fuselage_weight
from RCAIDE.Methods.Weights.Buildups.Common   import compute_boom_weight
from RCAIDE.Methods.Weights.Buildups.Common   import compute_rotor_weight
from RCAIDE.Methods.Weights.Buildups.Common   import compute_wiring_weight
from RCAIDE.Methods.Weights.Buildups.Common   import compute_wing_weight
...
def compute_weight(config,
    ...
    for network in config.networks:
        ...
        # Payload Weight
        #-------------------------------------------------------------------------------
        if network.payload.origin[0][0] == 0:
            network.payload.origin[0][0]                              = 0.5 * length_scale
        weight.payload                                                += network.payload.mass_properties.mass * Units.kg
 
        #-------------------------------------------------------------------------------
        # Avionics Weight
        #-------------------------------------------------------------------------------
        if network.avionics.origin[0][0] == 0:
        ...
        for bus in network.busses: 
            ...
            rotors                  = bus.rotors 
            for rotor in rotors:      
                if type(rotor) == RCAIDE.Energy.Converters.Propeller:
                    ''' Propeller Weight '''  
                ...
                if (type(rotor) == RCAIDE.Energy.Converters.Lift_Rotor or type(rotor) == RCAIDE.Energy.Converters.Prop_Rotor) or type(rotor) == RCAIDE.Energy.Converters.Rotor:

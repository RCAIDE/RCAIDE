...
## @ingroup Methods-Aerodynamics-Airfoil
def import_airfoil_geometry(airfoil):
    ...
    Assumptions:
    Coordinate file of airfoil must be in either Selig or Lednicer airfoil format.
    This method automatically detects which format based on the first line of the data.
    ...
    data_block, lednicer_format = read_coordinate_file(airfoil_geometry_file)
    
    if lednicer_format:
        x_up_surf, y_up_surf, x_lo_surf, y_lo_surf = format_lednicer(data_block)
    else:
        x_up_surf, y_up_surf, x_lo_surf, y_lo_surf = format_selig(data_block)
...
def read_coordinate_file(airfoil_geometry_file):
    ...
    # Close the file
    f.close() 
    return data_block, lednicer_format

def format_lednicer(data_block):
    x_up_surf = []
    y_up_surf = []
    x_lo_surf = []
    y_lo_surf = []
    ...
    return x_up_surf, y_up_surf, x_lo_surf, y_lo_surf

def format_selig(data_block):
    x_up_surf_rev  = []
    y_up_surf_rev  = []
    x_lo_surf      = []
    y_lo_surf      = []
